...
export function isFullscreen() {
  if (document.fullscreenElement) return document.fullscreenElement === canvas;
...
export function setCanvasDrawCallback(func) {
  if (typeof func == 'function') drawCallback = func;
  else drawCallback = undefined;
}
...
export function setCanvasResizeCallback(func) {
  if (typeof func == 'function') canvasResizeCallback = func;
  else canvasResizeCallback = undefined;
}
...
export function setOptions(options) {
  if (options.mode !== undefined) mode = Number(options.mode);

  if (options.minFreq !== undefined) minFreq = options.minFreq;

  if (options.maxFreq !== undefined) maxFreq = options.maxFreq;

  if (options.gradient !== undefined) gradient = options.gradient;

  if (options.showBgColor !== undefined) showBgColor = options.showBgColor;

  if (options.showLeds !== undefined) showLeds = options.showLeds;

  if (options.showScale !== undefined) showScale = options.showScale;

  if (options.minDb !== undefined) analyzer.minDecibels = options.minDb;

  if (options.maxDb !== undefined) analyzer.maxDecibels = options.maxDb;

  if (options.showPeaks !== undefined) showPeaks = options.showPeaks;

  if (options.showFPS !== undefined) showFPS = options.showFPS;

  if (options.loRes !== undefined) loRes = options.loRes;

  if (options.fftSize !== undefined) analyzer.fftSize = options.fftSize;
...
export function registerGradient(name, options) {
  ...
  if (options.bgColor !== undefined) gradients[name].bgColor = options.bgColor;
  else gradients[name].bgColor = '#111';

  if (options.dir !== undefined) gradients[name].dir = options.dir;
...
function preCalcPosX() {
  ...
  if (mode == 0) {
    ...
    var pos,
      ...
      maxIndex = Math.min(
        Math.round((maxFreq * analyzer.fftSize) / audioCtx.sampleRate),
        analyzer.frequencyBinCount - 1,
      );
    ...
    switch (mode) {
      ...
      default:
        spaceV = Math.min(
          3,
          Math.max(2, (canvas.height / (180 * pixelRatio)) | 0),
        );
        ledOptions = {
          nLeds: 128,
    ...
    ledOptions.nLeds = Math.min(
      ledOptions.nLeds,
      (canvas.height / (ledOptions.spaceV * 2)) | 0,
    );
    ...
    while ((freq = c0 * root24 ** i) <= maxFreq) {
      if (freq >= minFreq && i % mode == 0) temperedScale.push(freq);
      i++;
    }
    ...
    var barSpace =
      (canvas.width - barWidth * temperedScale.length) /
      (temperedScale.length - 1);
    ...
    temperedScale.forEach(function(freq, index) {
      ...
      if (prevBin > 0 && prevBin + 1 <= bin) idx = prevBin + 1;
      else idx = bin;
      ...
      if (temperedScale[index + 1] !== undefined) {
        nextBin = Math.round(
          (temperedScale[index + 1] * analyzer.fftSize) / audioCtx.sampleRate,
        );
        ...
        if (nextBin - bin > 1) prevBin += Math.round((nextBin - bin) / 2);
          ...
          if (
            analyzerBars.length > 0 &&
            idx == analyzerBars[analyzerBars.length - 1].dataIdx
          ) {
            avg = true;
      ...
      ledsCtx.fillRect(
        analyzerBars[analyzerBars.length - 1].posX - ledOptions.spaceH / 2,
        0,
        ledOptions.spaceH,
        canvas.height,
      );
  ...
  if (mode > 0) {
    ...
    ledsCtx.fillRect(
      analyzerBars[analyzerBars.length - 1].posX +
        barWidth -
        ledOptions.spaceH / 2 +
        (mode < 8 ? 2 : 1),
      0,
    ...
    for (
      i = ledOptions.ledHeight;
      i < canvas.height;
      i += ledOptions.ledHeight + ledOptions.spaceV
    )
  ...
  freqLabels.forEach(label => {
    ...
    if (label.freq >= 1000) label.freq = label.freq / 1000 + 'k';
  });
}
...
function draw() {
  ...
  else if (isLedDisplay) canvasCtx.fillStyle = '#111';
  else canvasCtx.fillStyle = gradients[gradient].bgColor; // use background color defined by gradient
  ...
  for (i = 0; i < l; i++) {
    ...
    if (bar.endIdx == 0)
      ...
      if (bar.average) {
        ...
        for (j = bar.dataIdx; j <= bar.endIdx; j++) barHeight += dataArray[j];
    ...
    if (isLedDisplay)
      // normalize barHeight to match one of the "led" elements
      barHeight =
        (((barHeight / 255) * ledOptions.nLeds) | 0) *
        (ledOptions.ledHeight + ledOptions.spaceV);
    else barHeight = ((barHeight / 255) * canvas.height) | 0;
    ...
    if (isLedDisplay)
      canvasCtx.fillRect(
        bar.posX + ledOptions.spaceH / 2,
        canvas.height,
        barWidth,
        -barHeight,
      );
    else canvasCtx.fillRect(bar.posX, canvas.height, barWidth, -barHeight);
      ...
      if (showPeaks)
        if (isLedDisplay)
          canvasCtx.fillRect(
            bar.posX + ledOptions.spaceH / 2,
            (ledOptions.nLeds -
              (((bar.peak / canvas.height) * ledOptions.nLeds) | 0)) *
              (ledOptions.ledHeight + ledOptions.spaceV),
            barWidth,
            ledOptions.ledHeight,
          );
        else
          canvasCtx.fillRect(bar.posX, canvas.height - bar.peak, barWidth, 2);

      if (bar.hold) bar.hold--;
      else {
        bar.accel++;
        bar.peak -= bar.accel;
      }
  ...
  if (showScale) {
    ...
    freqLabels.forEach(label =>
      canvasCtx.fillText(label.freq, label.posX, canvas.height - size),
    );
  }
  ...
  if (drawCallback) drawCallback(canvas, canvasCtx, pixelRatio);
...
function generateGradients() {
  ...
  Object.keys(gradients).forEach(function(key) {
    ...
    else grad = canvasCtx.createLinearGradient(0, 0, 0, canvas.height);
    ...
    if (gradients[key].colorStops) {
      gradients[key].colorStops.forEach((colorInfo, index) => {
        if (typeof colorInfo == 'object')
          ...
          grad.addColorStop(
            index / (gradients[key].colorStops.length - 1),
            colorInfo,
          );
      });
    }
...
function setCanvas(reason) {
  ...
  if (pixelRatio == 2 && window.screen.height <= 540) pixelRatio = 1;
...
export function create(container, options = {}) {
  ...
  var audioSource;
  if (options.source) audioSource = connectAudio(options.source);
  ...
  gradient =
    options.gradient === undefined ? defaults.gradient : options.gradient;
  showBgColor =
    options.showBgColor === undefined
      ? defaults.showBgColor
      : options.showBgColor;
  showLeds =
    options.showLeds === undefined ? defaults.showLeds : options.showLeds;
  showScale =
    options.showScale === undefined ? defaults.showScale : options.showScale;
  showPeaks =
    options.showPeaks === undefined ? defaults.showPeaks : options.showPeaks;
  ...
  analyzer.fftSize =
    options.fftSize === undefined ? defaults.fftSize : options.fftSize;
  analyzer.smoothingTimeConstant =
    options.smoothing === undefined ? defaults.smoothing : options.smoothing;
  analyzer.minDecibels =
    options.minDb === undefined ? defaults.minDb : options.minDb;
  analyzer.maxDecibels =
    options.maxDb === undefined ? defaults.maxDb : options.maxDb;
